...
namespace VipcoQualityControl.Controllers
{
    ...
    public class RequireQualityControlController : GenericController<RequireQualityControl>
    {
        ...
        private async Task<bool> SendMail(RequireQualityControl HasData,int Option = 0,string AnyData = "")
        {
            if (Option == 0)
            {
            ...
            else if (Option == 2)
            {
                ...
                if (QcGroup != null)
                {
                    ...
                    if (QcGroup.Name.ToLower().IndexOf("welder") != -1)
                    {
                        if (!string.IsNullOrEmpty(QcGroup.Email))
                        {
                            if (QcGroup.Email.IndexOf(',') != -1)
                    ...
                    if (ListMail.Any())
                    {
                        var WorkGroupName = (await this.RepositoryGroupMis.GetFirstOrDefaultAsync(x => x, x => x.GroupMis == HasData.GroupMIS)).GroupDesc ?? "ไม่ระบุ";

                        var BodyMessage =
                            "<body style=font-size:11pt;font-family:Tahoma>" +
                            "<h4 style='color:steelblue;'>เมล์ฉบับนี้เป็นแจ้งเตือนจากระบบงาน VIPCO Quality control SYSTEM</h4>" +
                            $"เรียน หน่วยงานเชิ่อม" +
                            $"<p>เรื่อง ระบบตรวจพบการร้องยื่นคำร้องขอตรวจสอบคุณภาพ NDE หรือ ตรวจ Visual Examination(VT) จากหน่วยงาน \"{WorkGroupName}\"</p>" +
                            $"<p>เอกสารงานเลขที่ {HasData.RequireQualityNo} วันเวลาที่ร้องขอเข้าตรวจสอบ {HasData.RequireDate.ToString("dd/MM/yy HH:mm")}น.</p>" +
                            $"<p>โปรดดำเนินการระบุข้อมูลเพิ่มเติมเพื่อดำเนินการเรียกตรวจ NDE หรือ ตรวจ Visual Examination(VT) ในขั้นตอนต่อไปและ</p>" +
            ...
            else if (Option == 3)
            {
                ...
                if (ListMail.Any())
                {
                    ...
                    var BodyMessage = "<body style=font-size:11pt;font-family:Tahoma>" +
                                            ...
                                            $"เรียน คุณ{EmpName}" +
                                            $"<p>เรื่อง การเปิดคำขอตรวจคุณภาพงานเลขที่ {HasData.RequireQualityNo} วันเวลาที่ร้องขอเข้าตรวจสอบ {HasData.RequireDate.ToString("dd/MM/yy HH:mm")}น.</p>" +
                                            $"<p style='color:red;'><b>ณ.ขณะนี้ได้ถูกยกเลิกจากหน่วยงานผู้ดูแล คุณ{AnyData} พร้อมทั้งมีข้อความดังนี้</b></p></br>" +
                                            $"<p style='color:maroon;'>\"<i>{HasData.Remark}</i>\"</p>" +
                                            $"<p>หากมีข้อสงสัยเพิ่มเติมโปรดติดต่อ หน่วยงานผู้ดูแล</p><hr/>" +
                                            $"<p>\"คุณ{EmpName}\" " +
                                            $"สามารถเข้าไปตรวจติดตามข้อมูลได้ <a href='http://{Request.Host}/qualitycontrol/require-qc/link-email/{HasData.RequireQualityControlId}'>ที่นี้</a> </p>" +
                                            "<span style='color:steelblue;'>This mail auto generated by VIPCO Quality control SYSTEM. Do not reply this email.</span>" +
                                        "</body>";
            ...
            if (key > 0)
            {
                ...
                if (HasData != null)
                {
                    ...
                    var GenarateData = new RequireQualityControlViewModel()
                    {
                        ...
                        MasterLists = new List<MasterProjectListViewModel>()
                    };
                        ...
                        foreach (var item in HasDataMasterList)
                        {
                            if (item.MasterProjectList != null)
                            {
                                if (item.Quantity - item.PassQuantity > 0)
                                {
                                    ...
                                    GenarateData.MasterLists.Add(new MasterProjectListViewModel()
                                    {
                                        CreateDate = MapMaster.CreateDate,
                                        Creator = MapMaster.Creator,
                                        GradeMaterial1 = MapMaster.GradeMaterial1,
                                        GradeMaterial2 = MapMaster.GradeMaterial2,
                                        JointNumber = MapMaster.JointNumber,
                                        Thickness = MapMaster.Thickness,
                                        TypeMaterial1 = MapMaster.TypeMaterial1,
                                        TypeMaterial2 = MapMaster.TypeMaterial2,
        ...
        public async Task<IActionResult> RequireQualityControlWaiting([FromBody] OptionRequireQualityControl Option)
        {
            ...
            try
            {
                ...
                if (GetData.Any())
                {
                    ...
                    {
                        var GroupName = "Welding WorkGroup";
                        if (Option.Status == 2)
                            GroupName = await this.RepositoryGroupMis.GetFirstOrDefaultAsync(x => x.GroupDesc, x => x.GroupMis == Data.GroupMIS) ?? "ไม่ระบุ";

                        var WorkGroupQcName = Option.Status == 2 ? GroupName : $"{Data.WorkGroupQualityControl.Name ?? "No-Data"}";
        ...
        [HttpPost("CancelRequireQualityControl")]
        public async Task<IActionResult> CancelRequireQualityControl([FromBody] CancelRequireQuailtyControlViewModel data)
        {
            if (data != null)
            {
                var hasData = await this.repository.FindAsync(x => x.RequireQualityControlId == data.RequireQualityControlId);
                if (hasData != null)
                {
                    hasData.RequireStatus = RequireStatus.Cancel;
                    hasData.Remark = data.RemarkCancel;
                    hasData.ModifyDate = DateTime.Now;
                    hasData.Modifyer = data.ByUser;

                    var update = await this.repository.UpdateAsync(hasData, hasData.RequireQualityControlId);

                    if (update != null)
                        await SendMail(update, 3, data.EmployeeName);
